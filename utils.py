...
def random_ragged_spline(key, T_total, num_knots, poly_orders, deriv_orders,
    ...
    flat_knots = jnp.reshape(knots, (num_knots, -1))
    diffs = jnp.linalg.norm(jnp.diff(flat_knots, axis=0), axis=1)
    # Prevent division by zero: ensure diffs sum is positive and T elements are positive
    T = T_total * (diffs / jnp.maximum(jnp.sum(diffs), 1e-6))
    T = jnp.maximum(T, 1e-4) # Enforce minimum time interval
    t_knots = jnp.concatenate((jnp.array([0., ]),
                               jnp.cumsum(T))).at[-1].set(T_total)
    coefs = []
...
def rk38_step(func, h, x, t, *args):
    ...
    # def print_debug_once(val_xi, val_x):
    #     if not hasattr(print_debug_once, 'has_printed'):
    #         print_debug_once.has_printed = False
        
    #     if not print_debug_once.has_printed:
    #         print(f'xi = {val_xi}')
    #         print(f'x = {val_x}')
    #         print_debug_once.has_printed = True
    
    # jax.debug.callback(print_debug_once, x, x)
    s = 4
    A = jnp.array([
        [0,    0, 0, 0],
        [1/3,  0, 0, 0],
    ...
    def scan_fun(carry, cut):
        ...
        # jax.debug.callback(print_debug_once, xi, x)
        ki = func(xi, ti, *args)
        #jax.debug.print('ki = {}', ki)
        K = K.at[i].set(ki)
        carry = (x, t, h, K, *args)
        return carry, ki
    ...
    # Debug initial x inputs (ONLY ONCE)
    # import sys
    # def debug_print_once(val, label):
    #     if not hasattr(debug_print_once, 'printed'):
    #         debug_print_once.printed = set()
        
    #     if label not in debug_print_once.printed:
    #          print(f"[DEBUG] {label}: {val}")
    #          debug_print_once.printed.add(label)

    # jax.debug.callback(debug_print_once, x, "rk38_step INIT x")

    #jax.debug.print('x.size = {}', x.size)
    #jax.debug.print('jnp.zeros((s, x.size)) = {}', jnp.zeros((s, x.size)))
    #jax.debug.print('jnp.squeeze(jnp.zeros((s, x.size))) = {}', jnp.squeeze(jnp.zeros((s, x.size))))
    carry, K = jax.lax.scan(scan_fun, init_carry, (jnp.arange(s), A, b, c))
    xf = x + h*(K.T @ b)
    
    # import sys
    # def exit_callback(val):
    #     print("Exiting after rk38_step")
    #     sys.exit(0)
    # jax.debug.callback(exit_callback, xf)
...
def _odeint_ckpt(func, x0, ts, *args):
    #jax.debug.print('x0 init = {}', x0)
    def scan_fun(carry, t1):
        x0, t0, *args = carry
        # import sys
        # def print_x0_and_exit(val):
        #     # 호출 횟수 카운팅
        #     if not hasattr(print_x0_and_exit, 'count'):
        #         print_x0_and_exit.count = 0
            
        #     print_x0_and_exit.count += 1
        #     jax.debug.print('x0 = {}', x0)
            
        #     if print_x0_and_exit.count >= 2:
        #         sys.exit(0)
        
        # jax.debug.callback(print_x0_and_exit, x0)
        
        # jax.debug.print('t0 = {}', t0)
        # jax.debug.print('t1 = {}', t1)
        #jax.debug.print('args = {}', args)--
...
def odeint_ckpt(func, x0, ts, *args):
    ...
    # Debug flat_x0 (ONLY ONCE)
    # import sys
    # def debug_print_flat(val_flat_x0):
    #     if not hasattr(debug_print_flat, 'printed'):
    #          debug_print_flat.printed = False
        
    #     if not debug_print_flat.printed:
    #          print(f"[DEBUG] odeint_ckpt flat_x0: {val_flat_x0}")
    #          debug_print_flat.printed = True

    # jax.debug.callback(debug_print_flat, flat_x0)
    
    #jax.debug.print('flat_x0 = {}', flat_x0)
    ...
    #jax.debug.print('xs = {}', xs) 여기 사이가 문제인듯
    return xs
...
def odeint_fixed_step(func, x0, t0, t1, step_size, *args):
    ...
    # Debug odeint_fixed_step inputs (ONLY ONCE)
    # import sys
    # def debug_print_fixed(val_x0, val_ts):
    #     if not hasattr(debug_print_fixed, 'printed'):
    #         debug_print_fixed.printed = False
        
    #     if not debug_print_fixed.printed:
    #          print(f"[DEBUG] odeint_fixed_step x0: {val_x0}")
    #          print(f"[DEBUG] odeint_fixed_step ts: {val_ts}")
    #          debug_print_fixed.printed = True

    # jax.debug.callback(debug_print_fixed, x0, ts)
...
def tree_index_update(x_tree, i, y_tree):
    ...
    return jax.tree_util.tree_map(lambda x, y:
...
def tree_axpy(a, x_tree, y_tree):
    ...
    axpy = jax.tree_util.tree_map(lambda x, y: x + y, ax, y_tree)
    return axpy


def tree_dot(x_tree, y_tree):
    """Compute the dot products between children of two PyTrees."""
    xy = jax.tree_util.tree_map(lambda x, y: jnp.sum(x*y), x_tree, y_tree)
    return xy
...
def spline_dt(t, t_knots, coefs, order=1):
    """Compute time derivative of polynomial spline at time `t`."""
    num_polys = coefs.shape[0]
    
    i = jnp.clip(jnp.searchsorted(t_knots, t, side='left') - 1,
                 0, num_polys - 1)
    
    # Avoid division by zero if interval is zero (though shouldn't happen in valid spline)
    dt_interval = jnp.maximum(t_knots[i+1] - t_knots[i], 1e-6)
    tau = (t - t_knots[i]) / dt_interval
    
    c = coefs[i] # Shape (poly_order+1, ...)
    
    # Iterate to compute derivative coefficients
    # This loop is unrolled by JIT as order is usually static small int
    for _ in range(order):
        poly_order = c.shape[0] - 1
        if poly_order < 1:
            # Derivative of constant is zero
            # We need to preserve remaining dimensions
            c = jnp.zeros_like(c[:1])
            break
        
        # c[1] corresponds to tau^1, deriv is 1*c[1]*tau^0
        # c[k] corresponds to tau^k, deriv is k*c[k]*tau^(k-1)
        
        powers = jnp.arange(1, poly_order + 1)
        
        # Reshape powers for broadcasting against c[1:]
        # c shape: (poly_order, D1, D2...)
        shape_suffix = (1,) * (c.ndim - 1)
        p = powers.reshape((poly_order,) + shape_suffix)
        
        c = c[1:] * p
        
    # Evaluate polynomial at tau
    poly_order = c.shape[0] - 1
    powers = jnp.arange(poly_order + 1)
    
    # Calculate value
    val = jnp.tensordot(c, tau**powers, axes=(0, 0))
    
    # Chain rule: d^n/dt^n = (1/dt_interval)^n * d^n/dtau^n
    val = val / (dt_interval**order)
